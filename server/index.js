...
app.get("/treaties/:id", async (req, res) => {
  try {
    const treaty = await pool.query(
      `SELECT treaties.id, treaties.name, treaties.concluded, treaties.city, treaties.entered_into_force, status, english_text.text FROM treaties INNER JOIN english_text ON treaties.id = english_text.treaty_id WHERE treaties.id = $1`, [req.params.id]
    );
...
app.get("/organizations/:id", async (req, res) => {
  try {
    const treaties = await pool.query(
      `SELECT treaties.* FROM treaties INNER JOIN treaties_by_organization ON treaties.id = treaties_by_organization.treaty_id WHERE organization_id = $1`, [req.params.id]
    );
...
app.get("/participation/:id", async (req, res) => {
  try {
    const participationByTreaty = await pool.query(
      `SELECT treaties.name AS treaty_name, treaties.city AS city, treaties.concluded AS concluded, treaties.entered_into_force AS treaty_entry_into_force, treaties.status AS treaty_status, countries.name AS country_name, participation.* FROM participation INNER JOIN treaties ON participation.treaty_id = treaties.id INNER JOIN countries ON participation.country_id = countries.id WHERE treaties.id = $1`, [req.params.id]
    );
