...
const TreatiesForOrganization = () => {
  ...
  const filteredTreatiesForOrganization = treatiesForOrganization.filter((elem) => {
    return filterByName(organizationName, elem.id)

  })

  const listOfTreatiesForOrganization = filteredTreatiesForOrganization.map((elem) => {
    return (
  ...
  return (
    <div>
      <header>
        <h1>
      {organizationInfromation[organizationName].name}
        </h1>
      </header>
